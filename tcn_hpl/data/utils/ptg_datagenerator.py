...
def main(
    task: str, ptg_root: str, config_root: str, data_type: str, data_gen_yaml: str
):
    """
    ...
    reshuffle_datasets, augment, num_augs = (
        config["data_gen"]["reshuffle_datasets"],
        config["data_gen"]["augment"],
        config["data_gen"]["num_augs"],
    )
    ...
    output_data_dir = (
        f"{config['paths']['output_data_dir_root']}/{config['task']}/{exp_name}"
    )
    ...
    activity_config_path = (
        f"{config['paths']['activity_config_root']}/{config['task']}.yaml"
    )
    ...
    if reshuffle_datasets:
        ...
        if data_type == "pro":
            ...
            train_vidids = [
                dset.index.name_to_video[f"{task_name}-{index}"]["id"]
                for index in config["data_gen"]["train_vid_ids"]
                if f"{task_name}-{index}" in dset.index.name_to_video.keys()
            ]
            val_vivids = [
                dset.index.name_to_video[f"{task_name}-{index}"]["id"]
                for index in config["data_gen"]["val_vid_ids"]
                if f"{task_name}-{index}" in dset.index.name_to_video.keys()
            ]
            test_vivds = [
                dset.index.name_to_video[f"{task_name}-{index}"]["id"]
                for index in config["data_gen"]["test_vid_ids"]
                if f"{task_name}-{index}" in dset.index.name_to_video.keys()
            ]
            ...
            if config["data_gen"]["filter_black_gloves"]:
                vidids_black_gloves = [
                    dset.index.name_to_video[f"{task_name}-{index}"]["id"]
                    for index in config["data_gen"]["names_black_gloves"]
                    if f"{task_name}-{index}" in dset.index.name_to_video.keys()
                ]
            ...
            if config["data_gen"]["filter_blue_gloves"]:
                vidids_blue_gloves = [
                    dset.index.name_to_video[f"{task_name}-{index}"]["id"]
                    for index in config["data_gen"]["names_blue_gloves"]
                    if f"{task_name}-{index}" in dset.index.name_to_video.keys()
                ]
            ...
            train_vidids = [
                dset.index.name_to_video[vid_name]["id"]
                for vid_name in all_vids
                if dset.index.name_to_video[vid_name]["id"]
                in config["data_gen"]["train_vid_ids_bbn"]
            ]
            val_vivids = [
                dset.index.name_to_video[vid_name]["id"]
                for vid_name in all_vids
                if dset.index.name_to_video[vid_name]["id"]
                in config["data_gen"]["val_vid_ids_bbn"]
            ]
            test_vivds = [
                dset.index.name_to_video[vid_name]["id"]
                for vid_name in all_vids
                if dset.index.name_to_video[vid_name]["id"]
                in config["data_gen"]["test_vid_ids_bbn"]
            ]
        ...
        print(
            f"train num_images: {len(train_img_ids)}, val num_images: {len(val_img_ids)}, test num_images: {len(test_img_ids)}"
        )
    ...
    for dset, split in zip(
        [train_dset, val_dset, test_dset], ["train_activity", "val", "test"]
    ):
        for video_id in ub.ProgIter(dset.index.videos.keys()):
            ...
            if data_type == "pro":
                ...
                activity_gt_file = (
                    f"{video_root}/{video_name}.skill_labels_by_frame.txt"
                )
            elif data_type == "lab":
                activity_gt_file = (
                    f"{skill_data_root}/{video_name}.skill_labels_by_frame.txt"
                )
                if not os.path.exists(activity_gt_file):
                    print(
                        f"activity_gt_file {activity_gt_file} doesnt exists. Trying a different way"
                    )
            ...
            for index, img_id in enumerate(video_dset.index.imgs.keys()):
                ...
                video_dset.index.imgs[img_id]["activity_gt"] = activityi_gt_list[
                    frame_index
                ]
            ...
            for aug_index in range(num_augs):
                ...
                # groundtruth
                with open(f"{gt_dir}/{video_name_new}.txt", "w") as gt_f, open(
                    f"{frames_dir}/{video_name_new}.txt", "w"
                ) as frames_f:
